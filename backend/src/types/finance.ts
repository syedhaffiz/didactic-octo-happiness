...
// row are handled on the client. Aggregates (the Total row and the banner) are
// derived from `items`, so they reflect the active filters but never the page.
...
// Query filters. `tillDate` is the as-of date for the balances; each filter is a
// comma-joined multi-select (an omitted field means "All"). Shared by the
// Debtors table and the Debtors overview.
