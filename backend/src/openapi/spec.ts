...
export const openApiSpec = {
    ...
    "/finance/profitability/vessels/sales": {
      get: {
        ...
        parameters: [
          ...
          { name: "batchId", in: "query", schema: { type: "string" }, description: "Case-insensitive substring search on Batch ID." },
          { name: "vessel", in: "query", schema: { type: "string" }, description: "Case-insensitive substring search on Vessel." },
          { name: "segment", in: "query", schema: { type: "string" }, description: "Case-insensitive substring search on Segment." },
        ],
    ...
    "/finance/profitability/vessels/handling": {
      get: {
        ...
        parameters: [
          ...
          { name: "batchId", in: "query", schema: { type: "string" }, description: "Case-insensitive substring search on Batch ID." },
          { name: "vessel", in: "query", schema: { type: "string" }, description: "Case-insensitive substring search on Vessel." },
          { name: "customer", in: "query", schema: { type: "string" }, description: "Case-insensitive substring search on Customer (All / Sagarmala / CIF)." },
          { name: "port", in: "query", schema: { type: "string" }, description: "Case-insensitive substring search on Port (TPH)." },
        ],
    ...
    "/finance/profitability/vessels/sales/{batchId}": {
      get: {
        ...
        parameters: [
          ...
          { name: "batchId", in: "query", schema: { type: "string" }, description: "Case-insensitive substring search on Batch No." },
          { name: "customerName", in: "query", schema: { type: "string" }, description: "Case-insensitive substring search on Customer Name." },
          { name: "plantName", in: "query", schema: { type: "string" }, description: "Case-insensitive substring search on Plant Name." },
          { name: "tradeContractNo", in: "query", schema: { type: "string" }, description: "Case-insensitive substring search on Trade Contract No." },
        ],
  ...
  components: {
    ...
    schemas: {
      ...
      VesselSalesResponse: {
        type: "object",
        required: ["currency", "summary", "total", "items"],
        properties: {
          currency: { type: "string", enum: ["INR", "USD"] },
          summary: { $ref: "#/components/schemas/VesselSummary" },
          total: { type: "number", description: "Total rows before the search filter." },
      ...
      VesselHandlingResponse: {
        type: "object",
        required: ["currency", "category", "summary", "total", "items"],
        properties: {
          currency: { type: "string", enum: ["INR", "USD"] },
      ...
      SalesBatchDetailResponse: {
        type: "object",
        required: ["batchId", "summary", "total", "items"],
        properties: {
          batchId: { type: "string" },
      ...
      DebtorsResponse: {
        type: "object",
        required: ["currency", "periodLabel", "items"],
        properties: {
          currency: { type: "string", enum: ["INR", "USD"] },
          periodLabel: { type: "string", example: "Apr 25 : Feb 26" },
          items: {
            type: "array",
            items: { $ref: "#/components/schemas/DebtorRow" },
            description:
              "Full filtered result set. The client paginates it and derives the Total row + banner from these rows.",
          },
        },
      },
