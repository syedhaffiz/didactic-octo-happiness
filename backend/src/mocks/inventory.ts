...
import { intRange, pick, range, round, seedFromString, seeded } from "./rand.js";

// The screenshots are anchored to early May 2026; the system date matches that
// window so the page reads as "current". This anchor is used for series.
...
const lengthFor = (range: IndexRange, cadence: "Daily" | "Weekly") => {
  // Daily series sample every day; weekly every 7 days.
...
const buildIndex = (def: (typeof INDEX_DEFS)[number], r: IndexRange): PriceIndex => {
  ...
  const seed = seedFromString(`index:${def.code}:${r}`);
  ...
  // Pin the last point to the headline value so the visible "current" matches.
  if (points.length > 0) {
...
const buildKpis = (f: InventoryFilters): InventoryKpi[] => {
...
const buildCurrentInventory = (f: InventoryFilters): PortInventoryRow[] => {
  const noFilter = !f.port && !f.origin && !f.grade;
  const rng = seeded(seedFromString(`inv:level:${filterKey(f)}`));
  // When a port is selected, restrict to just that port.
...
const buildDispatchSummary = (f: InventoryFilters): DispatchSummary => {
  ...
  // The screenshot shows -95.26% — compute deterministically so it stays close.
...
const buildSales = (f: InventoryFilters): SalesMonth[] => {
...
const buildVessels = (
  ...
  return Array.from({ length: count }, (_, idx) => {
    ...
    // Spread BL/ETA dates around the anchor.
