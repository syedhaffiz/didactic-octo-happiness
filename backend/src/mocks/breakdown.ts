...
const segmentValue = (
  ...
  for (const m of months) {
    ...
    // Wide spread so segments look distinct (matches the Figma "86.6 / 24.5 / 4.6 / 54.9").
    const center =
      segment === "SNS" ? 80 :
      segment === "SEB" ? 25 :
      segment === "TPH" ? 5 :
...
export const buildBreakdown = (
  ...
  const breakdown: BreakdownItem[] = BREAKDOWN_SEGMENTS.map((segment) => ({
    segment,
    value: segmentValue(resource, segment, port, months.length ? months : [{ year: 2025, month: 4, label: "2025-04" }]),
    unit: "Cr" as const,
  }));
  ...
  const ledger = buildLedger(
    resource,
    port,
    months.length ? months : [{ year: 2025, month: 4, label: "2025-04" }],
  );
