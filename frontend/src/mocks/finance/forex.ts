// Mirror of backend/src/mocks/forex.ts. Kept in sync.

import type { ForexResponse, ForexRange } from "../../types/finance";
import { round, seedFromString, seeded } from "../rand";

const WEEKDAYS = ["MON", "TUE", "WED", "THU", "FRI", "SAT", "SUN"] as const;

export const buildForex = (
  range: ForexRange = "week",
  anchor: Date = new Date(),
): ForexResponse => {
  const rng = seeded(
    seedFromString(`forex:${range}:${anchor.getUTCFullYear()}-${anchor.getUTCMonth() + 1}`),
  );
  const base = 92.5;
  const amplitude = 1.6;
  ...
  return { range, points, exchangeRate, monthAverage };
};
